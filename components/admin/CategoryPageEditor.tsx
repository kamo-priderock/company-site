...
export const SLUG_LABEL: Record<CategoryPageSlug, string> = {
...
export interface CategoryPageEditorProps {
  slug: CategoryPageSlug;
  page: CategoryPagePublic;
  onChange: (page: CategoryPagePublic) => void;
  showSlugPicker?: boolean;
  onSlugChange?: (slug: CategoryPageSlug) => void;
  onReload?: () => void;
...
export function CategoryPageEditor({
  ...
  page,
  onChange,
  showSlugPicker = false,
  onSlugChange,
  onReload,
  onResetToDefaults,
  reloading = false,
}: CategoryPageEditorProps) {
  const p = page;
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          {showSlugPicker && onSlugChange ? (
            ...
            <span className="text-sm font-medium text-slate-700">
              Landing page:{" "}
              <span className="text-slate-900">{SLUG_LABEL[slug]}</span>{" "}
